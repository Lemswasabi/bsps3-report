...
modules, such as language models or acoustic models, which were used in previous
...
which are speaker-dependent. We develop a speech recognition system for the
Luxembourgish language based on Recurrent Neural Networks (RNNs) and compare its
performance to standard feed-forward neural networks. RNNs use their internal
state to operate on data time-series. Combined with connectionist temporal
classification (CTC) models, they represent the current state-of-the-art in ASR.
The accuracy to recognize Luxembourgish words obtained with gated RNNs, such as
long short-term memory (LSTM) and gated recurrent units (GRU), were both roughly
99\%.
